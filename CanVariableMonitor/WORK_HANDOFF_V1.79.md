...
- 兼容下载接口：`POST http://8.148.250.52/anyixt-cloud-device/utility-base-api/v1/device_config/download_file`。
- 文件名走 query 参数 `fileName`，版本号从 HTTP 响应头 `X-Version` 读取。
- `update_config.json` 支持 `downloadApiUrl`、`packageFileName`、`versionHeaderName` 等字段。
- 推荐服务器提供稳定包名，例如 `上位机监控_latest.zip`，每次替换为最新包，并更新响应头 `X-Version`；否则客户端无法提前知道下一版具体文件名。
- `PublishUnified.ps1` 会把 `update_config.json` 放进发布目录；若没有正式文件，则用 `update_config.example.json` 生成客户无感配置。
...
## 10. 2026-06-15 - V1.81 服务器更新验证包

- 本地 F 盘验证目录保持 `V1.80`，不要用统一发布脚本覆盖它，这样才能验证自动更新。
- 给服务器上传的主文件是 `CanVariableMonitor\release\上位机监控_latest.zip`，当前内容版本为 `V1.81`。
- 同目录保留版本归档包：`CanVariableMonitor\release\上位机监控_V1.81_20260615_003653.zip`。
- 下载接口模式下不强依赖 `update_manifest.json`，但 release 目录仍生成了一份用于记录和兼容旧模式。
- 服务器下载接口必须在返回 zip 时带响应头：`X-Version: V1.81`。
- 客户端请求方式：`POST http://8.148.250.52/anyixt-cloud-device/utility-base-api/v1/device_config/download_file?fileName=上位机监控_latest.zip`。
- 本次 `上位机监控_latest.zip` 的 SHA256：`729C86050A57646D6A923A831B53EA36EFEE4E5608B37C3467FE31A80CB02CF8`。
